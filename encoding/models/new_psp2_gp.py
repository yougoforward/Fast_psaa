...
class new_psp2_gp(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(new_psp2_gp, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = new_psp2_gpHead(2048, nclass, norm_layer, se_loss, self._up_kwargs)
        if aux:
...
def get_new_psp2_gp(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = new_psp2_gp(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
        from .model_store import get_model_file
        model.load_state_dict(torch.load(
            get_model_file('new_psp2_gp_%s_%s'%(backbone, acronyms[dataset]), root=root)))
    return model

def get_new_psp2_gp_resnet50_ade(pretrained=False, root='~/.encoding/models', **kwargs):
    r"""new_psp2_gp model from the paper `"Context Encoding for Semantic Segmentation"
    ...
    Examples
    --------
    >>> model = get_new_psp2_gp_resnet50_ade(pretrained=True)
    >>> print(model)
    """
...
class PyramidPooling(Module):
    ...
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        ...
        self.project = nn.Sequential(nn.Conv2d(in_channels=5*out_channels, out_channels=out_channels,
                      ...
                      norm_layer(out_channels),
                      nn.ReLU(True))
        self.gap = gap_Pooling(in_channels, out_channels, norm_layer)
