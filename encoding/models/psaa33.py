...
__all__ = ['psaa33Net', 'get_psaa33net']


class psaa33Net(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(psaa33Net, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = psaa33NetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class psaa33NetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
                 atrous_rates=(12, 24, 36)):
        super(psaa33NetHead, self).__init__()
        self.se_loss = se_loss
        inter_channels = in_channels // 4

        self.aa_psaa33 = psaa33_Module(in_channels, inter_channels, atrous_rates, norm_layer, up_kwargs)
        self.conv8 = nn.Sequential(nn.Dropout2d(0.1), nn.Conv2d(inter_channels, out_channels, 1))

    def forward(self, x):
        feat_sum = self.aa_psaa33(x)
        outputs = [self.conv8(feat_sum)]
        return tuple(outputs)


def psaa33Conv(in_channels, out_channels, atrous_rate, norm_layer):
    block = nn.Sequential(
...
class psaa33Pooling(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(psaa33Pooling, self).__init__()
...
class psaa33_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        ...
        self.b0 = nn.Sequential(
            ...
            norm_layer(out_channels),
            nn.ReLU(True))
        self.b1 = psaa33Conv(in_channels, out_channels, rate1, norm_layer)
        self.b2 = psaa33Conv(in_channels, out_channels, rate2, norm_layer)
        self.b3 = psaa33Conv(in_channels, out_channels, rate3, norm_layer)
        self.b4 = psaa33Pooling(in_channels, out_channels, norm_layer, up_kwargs)
        ...
        # self.gap = psaa33Pooling(out_channels, out_channels, norm_layer, up_kwargs)
...
def get_psaa33net(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = psaa33Net(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
