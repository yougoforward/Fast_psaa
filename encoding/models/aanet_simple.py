...
class AANetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
        ...
        self.conv51 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 1, padding=1, bias=False),
                                    norm_layer(inter_channels), nn.ReLU(True))
        self.conv52 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 1, padding=1, bias=False),
                                    norm_layer(inter_channels), nn.ReLU(True))
        self.conv53 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 1, padding=1, bias=False),
                                    norm_layer(inter_channels), nn.ReLU(True))
...
def get_aanet_fast(dataset='pascal_voc', backbone='resnet50', pretrained=False,
              root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = AANet(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
...
class PAM_Module(nn.Module):
    ...
    # Ref from SAGAN
    def __init__(self, in_dim, key_dim, out_dim, scale=1):
    ...
    def forward(self, x):
        ...
        proj_query = self.query_conv(x1).view(m_batchsize, self.key_dim, -1).permute(0, 2, 1)
        proj_key = self.key_conv(x1).view(m_batchsize, self.key_dim, -1)
        energy = torch.bmm(proj_query, proj_key)
        attention = self.softmax(energy)
        proj_value = self.value_conv(x1).view(m_batchsize, self.out_dim, -1)

        out = torch.bmm(proj_value, attention.permute(0, 2, 1))
        out = out.view(m_batchsize, self.out_dim, *x1.size()[2:])

        if self.scale > 1:
                out = F.upsample(input=out, size=(height, width), mode='bilinear', align_corners=True)
...
class topk_PAM_Module(nn.Module):
    ...
    # Ref from SAGAN
    def __init__(self, in_dim, key_dim, out_dim, topk=10, scale = 1):
    ...
    def forward(self, x):
        ...
        val, idx = torch.topk(energy, int(h * w * self.topk), dim=2, largest=True, sorted=False)
