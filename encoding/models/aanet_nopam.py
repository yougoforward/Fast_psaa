...
__all__ = ['AANet_nopam', 'get_aanet_nopam']


class AANet_nopam(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(AANet_nopam, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = AANet_nopamHead(2048, nclass, norm_layer, se_loss, self._up_kwargs)
        if aux:
...
def get_aanet_nopam(dataset='pascal_voc', backbone='resnet50', pretrained=False,
              root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = AANet_nopam(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
