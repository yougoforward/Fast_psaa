...
def get_aanet(dataset='pascal_voc', backbone='resnet50', pretrained=False,
...
class guided_CAM_Module(nn.Module):
    ...
    def __init__(self, in_dim, query_dim, out_dim):
        ...
        self.gamma = nn.Parameter(torch.zeros(1))
        self.softmax = nn.Softmax(dim=-1)
        self.query_conv_c = nn.Sequential(nn.Conv2d(in_channels=in_dim, out_channels=query_dim, kernel_size=1, bias=False),nn.BatchNorm2d(query_dim), nn.ReLU(),nn.Dropout2d(0.1))
