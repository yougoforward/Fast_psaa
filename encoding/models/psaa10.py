...
__all__ = ['psaa10Net', 'get_psaa10net']


class psaa10Net(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(psaa10Net, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = psaa10NetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class psaa10NetHead(nn.Module):
    ...
    def forward(self, x):
        feat_sum, gap_feat = self.aa_psaa10(x)
...
def psaa10Conv(in_channels, out_channels, atrous_rate, norm_layer):
    block = nn.Sequential(
...
class psaa10Pooling(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(psaa10Pooling, self).__init__()
...
class psaa10_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        ...
        # self.gap = psaa10Pooling(out_channels, out_channels, norm_layer, up_kwargs)
    ...
    def forward(self, x):
        ...
        return out, gp
def get_psaa10net(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = psaa10Net(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
...
#         proj_query = self.query_conv(xp).view(m_batchsize, -1, wp*hp).permute(0, 2, 1)
...
#         out = out.view(m_batchsize, C, hp, wp)
