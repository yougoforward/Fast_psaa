...
__all__ = ['ASPOC_GSECAM_DU_Net', 'get_aspoc_gsecam_dunet']


class ASPOC_GSECAM_DU_Net(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(ASPOC_GSECAM_DU_Net, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = ASPOC_GSECAM_DU_NetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'],
...
class ASPOC_GSECAM_DU_NetHead(nn.Module):
    def __init__(self, in_channels, num_classes, norm_layer, se_loss, jpu=False, up_kwargs=None,
        ...
        self.conv8 = nn.Sequential(nn.Dropout2d(0.1), nn.Conv2d(256, num_classes, 1))
...
def get_aspoc_gsecam_dunet(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                        root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = ASPOC_GSECAM_DU_Net(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
