...
__all__ = ['aanet_encnet', 'get_aanet_encnet']


class aanet_encnet(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(aanet_encnet, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = aanet_encnetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class aanet_encnetHead(nn.Module):
    ...
    def forward(self, x):
        # ssa
        feat1 = self.conv5a(x)
        outs = list(self.encmodule(feat1))
        sa_conv = outs[0]
        ...
        if self.se_loss:
            ...
            outputs.append(self.selayer(torch.squeeze(gap_feat))+outs[1])
        else:
            outputs = [self.conv8(feat_sum)]
...
def get_aanet_encnet(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                   root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = aanet_encnet(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
