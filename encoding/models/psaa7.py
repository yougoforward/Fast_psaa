...
__all__ = ['psaa7Net', 'get_psaa7net']


class psaa7Net(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(psaa7Net, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = psaa7NetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class psaa7NetHead(nn.Module):
    ...
    def forward(self, x):
        feat_sum, gap_feat = self.aa_psaa7(x)
...
def psaa7Conv(in_channels, out_channels, atrous_rate, norm_layer):
    block = nn.Sequential(
...
class psaa7Pooling(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(psaa7Pooling, self).__init__()
...
class psaa7_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        super(psaa7_Module, self).__init__()
        ...
        self.b0 = nn.Sequential(
            ...
            norm_layer(out_channels),
            nn.ReLU(True))
        self.b1 = psaa7Conv(in_channels, out_channels, rate1, norm_layer)
        self.b2 = psaa7Conv(in_channels, out_channels, rate2, norm_layer)
        self.b3 = psaa7Conv(in_channels, out_channels, rate3, norm_layer)
        self.b4 = psaa7Pooling(in_channels, out_channels, norm_layer, up_kwargs)
        ...
        # self.gap = psaa7Pooling(5*out_channels, out_channels, norm_layer, up_kwargs)
        self.gap = nn.Sequential(nn.AdaptiveAvgPool2d(1),
                            nn.Conv2d(in_channels, out_channels, 1, bias=False),
                            norm_layer(out_channels),
                            nn.ReLU(True))
...
def get_psaa7net(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = psaa7Net(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
