...
__all__ = ['psp5Net', 'get_psp5net']


class psp5Net(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(psp5Net, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = psp5NetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class psp5NetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
                 atrous_rates=(12, 24, 36)):
        super(psp5NetHead, self).__init__()
        self.se_loss = se_loss
        inter_channels = in_channels // 4

        self.aa_psp5 = psp5_Module(in_channels, inter_channels, atrous_rates, norm_layer, up_kwargs)
    ...
    def forward(self, x):
        feat_sum, gap_feat = self.aa_psp5(x)
...
def psp5Conv(in_channels, out_channels, atrous_rate, norm_layer):
    block = nn.Sequential(
...
class psp5Pooling(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(psp5Pooling, self).__init__()
...
class psp5_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        ...
        self.b0 = nn.Sequential(
            ...
            norm_layer(out_channels),
            nn.ReLU(True))
        self.b1 = psp5Conv(in_channels, out_channels, rate1, norm_layer)
        self.b2 = psp5Conv(in_channels, out_channels, rate2, norm_layer)
        self.b3 = psp5Conv(in_channels, out_channels, rate3, norm_layer)
        # self.b4 = psp5Conv(in_channels, out_channels, rate4, norm_layer)
        # self.b4 = psp5Pooling(in_channels, out_channels, norm_layer, up_kwargs)
...
def get_psp5net(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = psp5Net(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
