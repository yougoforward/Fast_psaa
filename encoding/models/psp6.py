...
__all__ = ['psp6Net', 'get_psp6net']


class psp6Net(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(psp6Net, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = psp6NetHead(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class psp6NetHead(nn.Module):
    ...
    def forward(self, x):
        feat_sum, gap_feat = self.aa_psp6(x)
...
def psp6Conv(in_channels, out_channels, atrous_rate, norm_layer):
    block = nn.Sequential(
...
class psp6Pooling(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(psp6Pooling, self).__init__()
...
class psp6_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        ...
        self.b0 = nn.Sequential(
            ...
            norm_layer(out_channels),
            nn.ReLU(True))
        self.b1 = psp6Conv(in_channels, out_channels, rate1, norm_layer)
        self.b2 = psp6Conv(in_channels, out_channels, rate2, norm_layer)
        self.b3 = psp6Conv(in_channels, out_channels, rate3, norm_layer)
        ...
        # self.b4 = psp6Conv(in_channels, out_channels, rate4, norm_layer)
        # self.b4 = psp6Pooling(in_channels, out_channels, norm_layer, up_kwargs)
...
def get_psp6net(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = psp6Net(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
